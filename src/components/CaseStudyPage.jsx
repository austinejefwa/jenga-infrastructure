  ...
  if (!study) {
    return (
      <div className="min-h-screen bg-ink flex flex-col items-center justify-center px-6">
        ...
        <Link to="/" className="font-mono text-xs text-sea underline" data-cursor-pointer>
          Back to home
        </Link>
      </div>
    )
    ...
    <div className="min-h-screen bg-ink pt-32 pb-20 px-6 md:px-10">
      <div className="max-w-4xl mx-auto">
        <Link
          to="/#projects"
          data-cursor-pointer
          className="font-mono text-xs text-concrete/50 hover:text-amber transition-colors mb-10 inline-block"
        >
          ← Back to projects
        </Link>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <div className="aspect-video border border-steel/60 bg-harbor blueprint-grid mb-10 flex items-center justify-center">
            <p className="font-mono text-xs text-concrete/30">Project gallery — placeholder</p>
          </div>

          <p className="text-concrete/70 text-base md:text-lg leading-relaxed mb-12 max-w-2xl">
            {study.summary}
          </p>
