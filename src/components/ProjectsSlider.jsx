  ...
  return (
    <Link to={`/projects/${project.slug}`} data-cursor-pointer>
      ...
      >
        ...
        >
          <p className="font-mono text-[10px] tracking-widest uppercase text-amber mb-2">{project.type}</p>
          ...
          <p className="font-mono text-[10px] text-sea mt-3 uppercase tracking-wider">View case study →</p>
        </motion.div>

        <div className="absolute top-4 right-4 font-mono text-[10px] text-concrete/70">
    ...
    <section id="projects" ref={containerRef} className="relative h-[300vh]">
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden" style={{ perspective: 1000 }}>
        <div className="max-w-7xl mx-auto px-6 md:px-10 w-full mb-10">
          <p className="font-mono text-xs tracking-widest uppercase text-sea mb-3">Site Survey — Selected Works</p>
          <h2 className="font-display text-3xl md:text-5xl text-concrete">Projects on the ground</h2>
        </div>
